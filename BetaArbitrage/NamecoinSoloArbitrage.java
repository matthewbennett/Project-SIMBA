...
public class NamecoinSoloArbitrage {
  //main method
  ...
  */
  ...
  { 
    info = btce.getInfo();
      ...
      else if(currncey.equals("nmc/btc"))
        {
          price = btcTonmcTemp;
          pair = BTCE.Pairs.NMC_BTC;
          funds = info.info.funds.btc; 
        }
      else if(currncey.equals("nmc/usd"))
        {
          price = nmcToUsd;
  ...
  sell method 
  */
  public static boolean sellnmc() throws BTCEException, NullPointerException 
  {
      trade_type = BTCE.TradeType.SELL; // type of trade
      price = nmcToUsd;  // sell slightly lower than asking price to insure immedaite trade 
  ...
  public static boolean sellnmcToBtc() throws BTCEException, NullPointerException 
  {
      trade_type = BTCE.TradeType.SELL; // type of trade
      price = btcTonmcTemp;  // sell slightly lower than asking price to insure immedaite trade 
  ...
  */
  public static void checkForProfit() throws NullPointerException, BTCEException, InterruptedException
  {
    System.out.println();
      ...
      btcTonmcTemp = btce.getTicker(NMC_BTC).buy;
      btcTonmc = btcTonmcTemp * btcToUsd;
      nmcToUsd = btce.getTicker(NMC_USD).sell;
      remainder = nmcToUsd - btcTonmc;
     

    System.out.println("BTC/USD = "+btcToUsd);
    System.out.println("BTC/nmc temp =" +btcTonmcTemp);
    System.out.println("BTC/nmc ="+ btcTonmc);
    System.out.println("nmc/USD ="+nmcToUsd);
    System.out.println("remainder ="+ remainder);
    if(remainder > 0)
    {
      // IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT NEXT 5 LINES BELOW!!!!!
      profit = (9 / btcTonmc) * remainder;
      ...
      if(profit > 0.04)
      {
        ...
        {
         info = btce.getInfo();
         accountnmc = info.info.funds.nmc;
         currentFunds = currentFunds / btcTonmcTemp - 0.05;
         while(accountnmc < currentFunds)
         {
            Thread.sleep(1000);
            info = btce.getInfo();     
            accountnmc = info.info.funds.nmc;
            System.out.println("waiting for order #2 to be filled, nmc funds are currently "+accountnmc);
          }
        }
        else if(buy("nmc/btc") == true)
        {
         info = btce.getInfo();
         accountnmc = info.info.funds.nmc;
         currentFunds = currentFunds / btcTonmcTemp - 0.05;
      ...
      btcTonmcTemp = btce.getTicker(NMC_BTC).sell;
      btcTonmc = btcTonmcTemp * btcToUsd;
      nmcToUsd = btce.getTicker(NMC_USD).buy;
      remainder = btcTonmc - nmcToUsd;
      ...
      if(profit > 0.04)
      {
        if(buy("nmc/usd") == true)
        {
          info = btce.getInfo();
          accountnmc = info.info.funds.nmc;
          currentFunds = 9 / nmcToUsd - 0.05;

          while(accountnmc < currentFunds)
          {
            Thread.sleep(1000);
            info = btce.getInfo();
            accountnmc = info.info.funds.btc;
            System.out.println("waiting for order #1 to be filled, nmc funds are currently "+accountBtc);
          }
        }
        if(sellnmcToBtc() == true)
        {
         info = btce.getInfo();
         accountBtc = info.info.funds.btc;
         currentFunds = currentFunds * btcTonmcTemp - 0.05;
         while(accountBtc < currentFunds)
          {
            ...
            info = btce.getInfo();
            accountBtc = info.info.funds.btc;
            System.out.println("waiting for order #2 to be filled, btc funds are currently "+accountnmc);
          }
        }
        else if(sellnmcToBtc() == true)
        {
         info = btce.getInfo();
         accountBtc = info.info.funds.btc;
         currentFunds = currentFunds * btcTonmcTemp - 0.05;
